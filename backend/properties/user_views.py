...
def add_property(request):
    ...
    if serializer.is_valid():
        ...
        property_instance = Property(
            ...
            rent_duration=serializer.validated_data.get('rentDuration'),
            commission=serializer.validated_data.get('commission'),
            floor_plan=serializer.validated_data.get('floorPlan'),
            main_image_index=serializer.validated_data.get('mainImageIndex', 0),
        )
...
@api_view(["PATCH"])
@permission_classes([IsAuthenticated])
def update_property_step(request, property_id):
    property_instance = get_object_or_404(Property, property_id=property_id, user=request.user)
    serializer = AddPropertySerializer(property_instance, data=request.data, partial=True)
    if serializer.is_valid():
        serializer.save()  # This updates the fields provided in the payload
        
        # Check if this update is meant to finalize the property.
        if request.data.get("finalize", False):
            property_instance.is_draft = False
            property_instance.current_step = 7  # Adjust as needed for your final step
            property_instance.save(update_fields=['is_draft', 'current_step'])
            
            # Optionally, you can re-run any final processing or similar property calculations here.
